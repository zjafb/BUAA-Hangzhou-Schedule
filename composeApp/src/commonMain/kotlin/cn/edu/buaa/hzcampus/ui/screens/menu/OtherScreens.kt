...
private val ABOUT_USAGE_GUIDE =
    ...
    2. 课表
       ...
       课表页右上角「导出到系统日历」可把整学期课表生成日历文件，在分享面板里选「日历」导入后，上课会出现在系统日历中并提前 15 分钟提醒。
    ...
    5. 考试 / 成绩
       进入后按学期查看，顶部可切换学期。首页会显示最近一场考试的倒计时。
       成绩页顶部提供加权 GPA 统计（优先用成绩单里的官方绩点）、各学期 GPA 与分数分布图表，以及绩点模拟：可试算「新增一门课考多少分」或「已有课程调分」对 GPA 的影响。
    ...
    9. 校园指南
       内置杭州校区（中法未来科技学院）学习生活自助指南，含课程学习资料与校园生活信息；在应用内直接浏览，不用跳浏览器。

    10. 空间预约
        在浏览器中打开「空间预约管理平台」（该平台依赖钉钉环境，浏览器里可能需要先登录）。

    11. 课程查询 / 成绩查询
        普通功能的「课程查询」只列出每门课程及成绩、学分、学时等明细；
        高级功能的「成绩查询」只做 GPA 统计、各学期成绩分析与绩点模拟。两者共用一个学期选择。

    12. 自动评教
        一键完成学期末评教任务（可用性未知，待测试）。
...
private val ABOUT_CHANGELOG =
    """
    v1.0.3
    · 新增：课表一键导出到系统日历（逐周生成日程，自带提前 15 分钟提醒）
    · 新增：成绩页加权 GPA 统计、各学期 GPA 与分数分布图表、绩点模拟
    · 新增：成绩页「总学时」统计与逐门课程学时，口径与官网成绩页一致
    · 新增：首页考试倒计时、校历周次、节假日与调休标记
    · 新增：校园指南（内置杭州校区学习生活自助指南）
    · 修复：绩点改用北航官方公式 4 − 3 × (100 − 分数)² ÷ 1600，并忽略接口里占位的 0 绩点
    · 修复：启动时先闪一下登录页（已登录状态被短暂判为未登录）
    · 修复：课程提醒只响一次、重启后提醒丢失（声明精确闹钟权限、调度前不再误判授权、开机与覆盖安装后重排，一次排满未来 7 天并每天自动续排）
    · 调整：成绩页拆分为普通功能「课程查询」（逐门课程明细）与高级功能「成绩查询」（GPA 统计 / 成绩分析 / 绩点模拟）
    · 调整：空间预约不再在应用内嵌页面，改为用浏览器打开；课表导出按钮下方补「.ics导出」小字
    · 优化：登录统计改为后台上报，不再阻塞登录
