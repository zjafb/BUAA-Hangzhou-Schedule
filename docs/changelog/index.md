...
杭州版的发布说明和安装包请参见 [GitHub Releases](https://github.com/zjafb/BUAA-Hangzhou-Schedule/releases)。下方旧版记录保留上游项目历史。

## v1.0.4（杭州版）

**发布日期：** 2026-09-20

- 设置：课前提醒时间前移；邮件数量 10、15、20、30 与自定义按钮同排，窄屏或大字体时可横向滚动。
- 提醒：课程与今日计划共用全屏设置，息屏/锁屏请求亮屏全屏提示，解锁使用中显示横幅；增加权限状态、分类设置入口和测试按钮。按需求移除实验性实时通知/流体云。
- 邮件：新邮件优先，打开时更新未读红点，支持默认数量设置及继续加载历史邮件。
- 课表：自定义计划按实际起止时间占用课表格子。
- 成绩：GPA 与加权平均分突出显示，移除算术平均分展示，兼容紧凑学期编号，查询失败可重试。
- 稳定性：优化请求取消、并发、会话隔离和缓存失效；普通与高级功能卡片尺寸统一，保留高级功能底部提示。

手机需允许本应用后台运行及横幅通知。系统后台限制可能导致离开应用后提醒延迟；学校服务器异常仍可能影响在线查询。

[下载 v1.0.4](https://github.com/zjafb/BUAA-Hangzhou-Schedule/releases/tag/v1.0.4)
